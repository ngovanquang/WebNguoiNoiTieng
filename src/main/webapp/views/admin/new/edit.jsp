            ...
            <div class="row">
                <div class="col-xs-12">
                    ...
                    <form id="formSubmit">
                        ...
                        <div class="form-group">
                            <label class="col-sm-3 control-label no-padding-right">Danh Nhân</label>
                            <div class="col-sm-9">
                                <select class="form-control" id="celebritycode" name="celebritycode">
                                    <c:if test="${empty model.celebritycode}">
                                        <option value="">Danh nhân</option>
                                        <c:forEach var="item" items="${celebrities}">
                                            <option value="${item.code}">${item.name}</option>
                                        </c:forEach>
                                    </c:if>
                                    <c:if test="${not empty model.celebritycode}">
                                        <option value="">Danh Nhân</option>
                                        <c:forEach var="item" items="${celebrities}">
                                            <option value="${item.code}" <c:if test="${item.code == model.celebritycode}">selected="selected"</c:if>>
                                                    ${item.name}
                                            </option>
                                        </c:forEach>
                                    </c:if>
                        ...
                        <br/>
                        <div class="form-group">
                            <label class="col-sm-3 control-label no-padding-right">Hình nền</label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="headimg" name="headimg" value="${model.headimg}"/>
                            </div>
                        </div>
                        <br/>
                        <br/>
    ...
    function addNew(data) {
        $.ajax({
            ...
            error: function (error) {
                window.location.href = "${NewURL}?type=list&maxPageItem=8&page=1&message=error_system";
            }
        });
    }
